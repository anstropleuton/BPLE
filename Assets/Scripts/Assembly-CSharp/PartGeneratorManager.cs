		...
		public void UpdateParts()
		{
			...
			foreach (PartData part5 in m_runtimeContraption.Parts)
			{
				...
				if (num4 < 0f)
				{
					...
					for (int l = 0; l < part2.transform.childCount; l++)
					{
						part2.transform.GetChild(l).gameObject.tag = "Contraption";
					}
			...
			foreach (PartData item2 in parts)
			{
				...
				{
					BasePart part3 = item2.Part;
					part3.ConnectedComponent = item2.ComponentIndex;
					part3.StrictConnectedComponent = item2.ComponentIndex;
					list.Add(part3);
					part3.Initialize();
					part3.PostInitialize();
				}
			}
			if (list.Count != 0)
			{
				...
				foreach (BasePart item3 in list)
				{
					...
					item3.StrictConnectedComponent = -1;
				}
			}
